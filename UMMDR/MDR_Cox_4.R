...
Cox.MDR <- function(snp.dat.part, SS, X, method){
  death <- SS[,2] 
  time <- SS[,1]
  X <- X
  ...
  ## Find the sign of the Martingale residuals for each cells
  sur <- Surv(time, death == 1) # Fit Surv model
  if ( is.null(X) == TRUE ){
    # Fit Cox Regression WITHOUT object from Surv
    cox <- coxph(sur ~ 1) 
  } else {
    # Fit Cox Regression WITH object from Surv
    cox <- coxph(sur ~ X) # Fit Cox Regression with object from Surv
  }
  ## Extract Martingale residuals from Cox regression model
  Mt.resi <- unname(residuals(cox, type="martingale"))  
  ...
  for(cel in 1:cells.num){
    ...
    Cell.sum <- sum(na.omit(Mt.resi[temp.ids]))
    if( Cell.sum > 0 ){        
  ...
  if (method == 'BA'){
    ...
    TP <- length(intersect(which(Mt.resi <= 0), which(G == 1))) 
    FN <- length(intersect(which(Mt.resi <= 0), which(G == 2)))
    FP <- length(intersect(which(Mt.resi > 0), which(G == 1)))
    TN <- length(intersect(which(Mt.resi > 0), which(G == 2)))
...
CV.Cox.MDR <- function(snp.dat, SS, X, nway, method, nfold){
  snp.dat <- snp.dat
  N <- dim(SS)[1]
  obs.id <- 1:N
  ind <- which(SS[,2] == 1)
  ...
  for (combi in 1:ncombi){ 
    ...
    for (cv in 1:nfold){
      ...
      # MDR for training #
      tmp <- Cox.MDR(train.snp.pair, train.SS, train.X, method)
      CV.train[combi,cv] <- tmp$stat
 
      # HL membership for testing #
      CV.test.HL[test.id] <- Cox.MDR(tmp.pair, SS, X, method)$HL[test.id]
    }
